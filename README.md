...
- `重置牛` 重置牛老婆次数，也可@用户重置别人的次数，失败禁言
- `切换ntr开关状态` 管理员命令，开启/关闭牛老婆功能
- `换老婆` 重新抽取老婆
- `重置换` 重置换老婆次数，其余同上，与重置牛共享次数
- `交换老婆` @用户和对方交换老婆 
- `同意交换` @用户同意
- `拒绝交换` @用户拒绝
- `查看交换请求` 查看交换老婆请求
...
v1.5.9使用!代替#拼接老婆出处与名称，解决图床访问问题。
...
v1.6.4优化老婆名称显示，过滤路径前缀。

v1.6.5重大更新：添加并发锁保护，支持多用户同时操作；合并记录文件；添加老婆帮助命令；支持前缀开关配置；代码结构优化。

v1.6.6修改昵称匹配方式为完整昵称匹配。
